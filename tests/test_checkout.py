...
from mychichair.checkout import views
from mychichair.checkout.core import STORAGE_SESSION_KEY, Checkout
from mychichair.shipping.models import ShippingMethodCountry
from mychichair.userprofile.models import Address
...
def test_checkout_shipping_address_with_storage(address_objects, shipping, monkeypatch):
    monkeypatch.setattr('mychichair.checkout.core.Address.objects', address_objects)
    checkout = Checkout(Mock(), AnonymousUser(), 'tracking_code')
...
def test_checkout_shipping_method(shipping_address, shipping_method, value, monkeypatch):
    ...
    monkeypatch.setattr('mychichair.checkout.core.ShippingMethodCountry.objects', queryset)
    checkout = Checkout(Mock(), AnonymousUser(), 'tracking_code')
