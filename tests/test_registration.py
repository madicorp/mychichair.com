...
from mychichair.registration.forms import OAuth2CallbackForm
from mychichair.registration.utils import (FACEBOOK, GOOGLE, FacebookClient,
                                       GoogleClient, OAuth2Client,
                                       OAuth2RequestAuthorizer, parse_response)
from mychichair.registration.views import change_email, oauth_callback
...
class BaseCommunicationTestCase(TestCase):
    ...
    def setUp(self):
        self.parse_mock = patch(
            'mychichair.registration.utils.parse_response').start()
        self.requests_mock = patch(
            'mychichair.registration.utils.requests').start()
...
class CallbackTestCase(TestCase):
    ...
    def setUp(self):
        patcher = patch(
            'mychichair.registration.forms.get_client_class_for_service')
        self.getter_mock = patcher.start()
        patcher = patch('mychichair.registration.forms.authenticate')
    ...
    @patch('mychichair.registration.forms.ExternalUserData')
    @patch('mychichair.registration.forms.User')
...
class EmailChangeTestCase(TestCase):
    ...
    @patch('mychichair.registration.views.now')
    @patch('mychichair.registration.views.EmailChangeRequest.objects.get')
