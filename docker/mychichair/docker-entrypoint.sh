...
python manage.py makemigrations && python manage.py migrate --noinput && python manage.py runserver 0.0.0.0:8000
else
...
python manage.py makemigrations && python manage.py migrate --noinput && gunicorn mychichair.wsgi --bind 0.0.0.0:8000
fi